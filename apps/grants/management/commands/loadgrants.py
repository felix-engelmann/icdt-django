    ...
    def handle(self, *args, **options):
        ...
        with open(options["file"]) as f:
            ...
            for row in tqdm(fcsv):
                ...
                spon = Sponsor.objects.get_or_create(
                    name=row["SPONSOR_NAME"], spn_id=row["SPONSOR_ID"]
                )[0]
                ...
                gr = Grant.objects.get_or_create(
                    type=t,
                    tid=tid,
                    status=status,
                    federal=fed,
                    ...
                    total=row["PROPOSAL_SPONSOR_TOTAL"],
                    title=row["PROPOSAL_TITLE"],
                )[0]
                Investigator.objects.get_or_create(
                    grant=gr, person=pi, type=Investigator.PI
                )
